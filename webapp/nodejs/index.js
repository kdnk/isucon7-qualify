...
const pool = mysql.createPool({
  ...
  password: process.env.ISUBATA_DB_PASSWORD || "password",
  database: "isubata",
  charset: "utf8mb4",
  insecureAuth: true,
});
...
function getInitialize(req, res) {
  return pool
    .query("DELETE FROM user WHERE id > 1000")
...
function dbGetUser(conn, userId) {
  return conn.query("SELECT * FROM user WHERE id = ?", [userId]).then(([result]) => result);
}

function dbAddMessage(conn, channelId, userId, content) {
  return conn.query("INSERT INTO message (channel_id, user_id, content, created_at) VALUES (?, ?, ?, NOW())", [
    channelId,
    userId,
    content,
  ]);
...
function loginRequired(req, res, next) {
  ...
  return dbGetUser(pool, req.userId).then(user => {
    req.user = user;
    next();
  });
}
...
function postLogin(req, res) {
  return pool.query("SELECT * FROM user WHERE name = ?", [req.body.name]).then(([row]) => {
    if (!row) {
      res.status(403).end();
      return;
    }
...
function postMessage(req, res) {
  ...
  return dbGetUser(pool, userId).then(user => {
    ...
    return dbAddMessage(pool, channel_id, userId, message).then(() => res.status(204).end(""));
  });
}
...
function getMessage(req, res) {
  ...
  return pool
    .query("SELECT * FROM message WHERE id > ? AND channel_id = ? ORDER BY id DESC LIMIT 100", [
      last_message_id,
      channel_id,
    ])
    .then(rows => {
...
function getHistory(req, res) {
  ...
  const N = 20;
  return pool.query("SELECT COUNT(*) as cnt FROM message WHERE channel_id = ?", [channelId]).then(([row2]) => {
    const cnt = row2.cnt;
...
function getProfile(req, res) {
  ...
  return getChannelListInfo(pool).then(({ channels }) => {
    return pool.query("SELECT * FROM user WHERE name = ?", [userName]).then(([user]) => {
      if (!user) {
        res.status(404).end();
        return;
      }
...
function postAddChannel(req, res) {
  ...
  return pool
    .query("INSERT INTO channel (name, description, updated_at, created_at) VALUES (?, ?, NOW(), NOW())", [
      name,
      description,
    ])
    .then(({ insertId }) => {
